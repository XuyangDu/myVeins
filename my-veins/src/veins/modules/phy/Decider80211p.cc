...
simtime_t Decider80211p::processNewSignal(AirFrame* msg)
{
    ...
    if (!currentSignal.first and signal.getIsDest()) {
        currentSignal.first = frame;

        EV << "Receiving currentSignal Id " << currentSignal.first->getId() << std::endl;
        ...
        EV_TRACE << "AirFrame: " << frame->getId() << " -> Already synced to another AirFrame. Treating AirFrame as interference." << std::endl;
        return signal.getReceptionEnd();
    }
}
    ...
    if (frame == currentSignal.first) {
        if (result->isSignalCorrect()) {
            ...
            if (frame->getUnderMinPowerLevel()) {
                ...
                phy->sendControlMsgToMac(new cMessage("Error", Decider80211p::BLOCKED));
            }
...
void Decider80211p::switchToTx()
{
    if (currentSignal.first != 0) {
        ...
        if (allowTxDuringRx) {
            ...
            EV << "currentSignal Id " << currentSignal.first->getId() << std::endl;
