...
TraCIMobility* Mac1609_4::getMobility(cModule* nic)
{
    cModule* ccModule = nic->getParentModule()->getSubmodule("veinsmobility");
    if (ccModule)
        return dynamic_cast<TraCIMobility*>(ccModule);
    else {
        throw cRuntimeError("Can't find Mobility!");
    }
}

BaseConnectionManager* Mac1609_4::getConnectionManager(cModule* nic)
{
    cModule* ccModule = nic->getParentModule()->getParentModule()->getSubmodule("connectionManager");
    if (ccModule)
        return dynamic_cast<BaseConnectionManager*>(ccModule);
    else {
        throw cRuntimeError("Can't find ConnectionManager!");
    }
}
...
void Mac1609_4::initialize(int stage)
{
    ...
    if (stage == 0) {
        ...
        Mob = getMobility(getParentModule());
        cc = getConnectionManager(getParentModule());
    }
}
...
void Mac1609_4::handleSelfMsg(cMessage* msg)
{
    ...
    if (msg == nextChannelSwitch) {
        ...
        mac->setIrsInfo(pktToSend->getIrsInfo());
...
void Mac1609_4::handleLowerControl(cMessage* msg)
{
    if (msg->getKind() == MacToPhyInterface::PHY_RX_START) {
        ...
        EV << "RX_OVER Succeed Node " << getParentModule()->getParentModule()->getIndex() << " change from " << phy->getRadioState() << " to ";
        phy->setRadioState(Radio::SLEEP);
        EV << phy->getRadioState() << std::endl;
    }
        ...
        EV << "TX_OVER Node " << getParentModule()->getParentModule()->getIndex() << " change from " << phy->getRadioState() << " to ";
        phy->setRadioState(Radio::SLEEP);
        EV << phy->getRadioState() << std::endl;
    ...
    else if (msg->getKind() == Decider80211p::BITERROR || msg->getKind() == Decider80211p::COLLISION || msg->getKind() == Decider80211p::BLOCKED) {

        EV << "RX_OVER Failed Node " << getParentModule()->getParentModule()->getIndex() << " change from " << phy->getRadioState() << " to ";
        phy->setRadioState(Radio::SLEEP);
        EV << phy->getRadioState() << std::endl;
...
void Mac1609_4::sendFrame(Mac80211Pkt* frame, simtime_t delay, Channel channelNr, MCS mcs, double txPower_mW)
{
    ...
    // Obtain the position of the sender and receiver for beam alignment 
    LAddress::L2Type nicIdDest = frame->getDestAddr();
    LAddress::L2Type nicIdSrc = frame->getSrcAddr();
    Coord posDest = cc->getNicPos(nicIdDest);
    Coord posSrc = cc->getNicPos(nicIdSrc);

    IRSInfo irsInfo = frame->getIrsInfo();
    if (irsInfo.getIRSIndex() < 0) {
        // Set the antenna direction of the sender
        Coord orientation = (posDest - posSrc) / sqrt(posDest.sqrdist(posSrc));
        Mob->setOrientationByVector(orientation);

        // Set the antenna direction of the receiver
        cModule* nicDest = getSimulation()->getModule(nicIdDest);
        orientation = (posSrc - posDest) / sqrt(posDest.sqrdist(posSrc));
        TraCIMobility* MobDest = dynamic_cast<TraCIMobility*>(nicDest->getParentModule()->getSubmodule("veinsmobility"));
        MobDest->setOrientationByVector(orientation);
    }
    else {
        PhyIRS* phyIRS = dynamic_cast<PhyIRS*>(getParentModule()->getParentModule()->getParentModule()->getSubmodule("irs", irsInfo.getIRSIndex())->getSubmodule("phy"));
        Coord posIRS = phyIRS->pos;

        // Set the antenna direction of the sender
        Coord orientation = (posIRS - posSrc) / sqrt(posIRS.sqrdist(posSrc));
        Mob->setOrientationByVector(orientation);

        // Set the antenna direction of the receiver
        cModule* nicDest = getSimulation()->getModule(nicIdDest);
        orientation = (posIRS - posDest) / sqrt(posDest.sqrdist(posIRS));
        TraCIMobility* MobDest = dynamic_cast<TraCIMobility*>(nicDest->getParentModule()->getSubmodule("veinsmobility"));
        MobDest->setOrientationByVector(orientation);
    }
