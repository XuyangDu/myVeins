...
void BasePhyLayer::handleUpperMessage(cMessage* msg)
{
    ...
    EV << "Node " << getParentModule()->getParentModule()->getIndex() << " sending currentSignal Id " << frame->getId() << std::endl;
...
unique_ptr<AirFrame> BasePhyLayer::encapsMsg(cPacket* macPkt)
{
    ...
    Mac80211Pkt* tmp = check_and_cast<Mac80211Pkt*>(macPkt);
    frame->setIrsInfo(tmp->getIrsInfo());
...
void BasePhyLayer::sendMessageDown(AirFrame* msg)
{
    ...
    if (irsId < 0) {
        ...
        cGate* gate = phyirs->gate("radioIn")->getPathStartGate();

        Coord senderPos = antennaPosition.getPositionAt();
        Coord receiverPos = phyirs->antennaPosition.getPositionAt();
...
void BasePhyLayer::filterSignal(AirFrame* frame)
{
    ...
    if (irsInfo.getDestAddr() == getParentModule()->getId()) {
        signal.setIsDest(true);
    }
    ...
    if (irsInfo.getIRSIndex() < 0) {
        ...
        // add position information to signal
        signal.setSenderPoa({senderPosition, senderOrientation, antenna});
        signal.setReceiverPoa({receiverPosition, receiverOrientation, antenna});
